...
cat $traceImageFile | awk -v tmax="$tmax" '$2 <=tmax {print $1,$2,$3}' > $traceImage_xFile
cat $traceImageFile | awk -v tmax="$tmax" '$2 <=tmax {print $1,$2,$4}' > $traceImage_zFile
./plot2DField.sh $traceImage_x S '-CGMT_gray.cpt -Iz' 1E-5 m $dx X 1E-6 m $dt Time 1E-9 s on
./plot2DField.sh $traceImage_z S '-CGMT_gray.cpt -Iz' 1E-5 m $dx X 1E-6 m $dt Time 1E-9 s on
rm $traceImage_xFile
rm $traceImage_zFile
exit
#--------------------------------------------------
